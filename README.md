...
Aktywacja zimnej bazy to nie problem wysyłki, tylko pomiaru i przekazania do handlowca. Platforma mailingowa powie, ile osób otworzyło maila, ale nie powie, że ta konkretna osoba kliknęła ofertę „strona za złotówkę” i że warto dzisiaj do niej zadzwonić. Bez tego zespół sprzedaży reaguje za późno albo w ogóle.

System domyka pętlę: 8 maili i 3 SMS-y na bazie ok. 3000 kontaktów. Kliknięcie w ofertę zapisuje datę i nazwę oferty w CRM kampanii (Notion), potem lead trafia na landing. Osobny kanał zbiera dostarczenia, otwarcia, odrzucenia i wypisy z platformy mailingowej, ale tylko z tej kampanii, żeby dane nie mieszały się z innymi wysyłkami na tym samym koncie. Drugi SMS idzie wyłącznie do osób, które otworzyły któregoś z maili. Wypis z SMS-ów wymaga potwierdzenia na stronie, dopiero potem kontakt ląduje na blacklistie.
...
| Pierwszy mail kampanii: cztery oferty do kliknięcia | SMS follow-up (wizualizacja na telefonie) |
|:---:|:---:|
| ![Mail 01](assets/mail-01.png) | ![SMS](assets/sms.png) |

| CRM kampanii: otwarcia, kliknięcia per oferta, wypisy | Alert zespołu po zapisie na spotkanie |
|:---:|:---:|
| ![Notion CRM](assets/notion.png) | ![ClickUp](assets/clickup.png) |

| Potwierdzenie wypisu z SMS (RODO) | Wyniki kampanii: pełny lejek sprzedaży |
|:---:|:---:|
...
- **SMS 2 tylko dla otwierających** - segment budujesz z danych CRM, nie z ogólnych statystyk platformy mailingowej
- **Blacklisty przed wysyłką** - wykluczasz zablokowane kontakty przed każdą turą, żeby nie wysyłać do osób, które się wypisały
- **SMS bez polskich znaków** - niższy koszt wysyłki; ponawianie przy braku środków i kontrola salda przed turą

### Tracking i CRM

- **Śledzenie kliknięć per oferta** - 6 ofert, data zapisana w CRM, potem przekierowanie na landing (własny endpoint `/track`)
- **Ochrona przed błędnymi zapisami** - pusty email, niewypełniony placeholder lub nieznana oferta: przekierowanie bez wpisu w CRM
- **Zdarzenia z platformy mailingowej** - dostarczenia, otwarcia, odrzucenia i wypisy; filtr kampanii odcina eventy z innych wysyłek
- **Notion jako CRM kampanii** - kolumny dostarczenia i otwarć (1-8), kliknięcia per oferta, wypisy, ocena jakości kontaktu 1-5
- **Bezpieczne tworzenie rekordów** - ponowne sprawdzenie kontaktu przed utworzeniem, żeby równoległe zdarzenia nie robiły duplikatów

### RODO i wypisy

- **Dwukrokowy wypis SMS** - strona potwierdzenia, dopiero potem zapis na blacklistę; ochrona przed botami i przypadkowym tapnięciem
- **Spójna blacklista** - wypis SMS blokuje dalsze SMS-y i obniża ocenę kontaktu; wypis z maila idzie przez mechanizm platformy mailingowej

### Przekazanie do zespołu

- **Monitorowanie kalendarza** - sprawdzanie co 2 minuty, okno 5 minut, horyzont 30 dni
- **Tylko zapisy klientów** - filtrowanie po kodach bookingowych w tytule wydarzenia, pomijanie gdy uczestnikiem jest handlowiec
- **Bez duplikatów powiadomień** - deduplikacja przetworzonych zapisów i blokada między workerami serwera
- **Alert na czacie zespołu** - wiadomość z linkiem do widoku CRM z nowym leadem (ClickUp Chat)
